...
var allCaches = [staticCache, serverCache, outSideCache]; //add

var cacheFiles = []; //add scope for /blog and /article so that it can skip /customer scope
...
self.addEventListener('activate', function (e) {
  ...
  e.waitUntil( // Get all the cache keys (keyList)
...
self.addEventListener('fetch', function (event) {
  ...
  if (requestUrl.origin === location.origin) {
    if (skipIfThese(requestUrl)) return;
...
function skipIfThese(requestUrl) {
  if (requestUrl.pathname.indexOf("/pwa") < 0 && requestUrl.pathname.indexOf("/mobile") < 0 && requestUrl.pathname.indexOf("/storage") < 0 && requestUrl.pathname.indexOf("/cache") < 0 && requestUrl.pathname.indexOf("/api/") < 0 && requestUrl.pathname.indexOf("/meduim-product-placeholder.png") < 0 && requestUrl.pathname.indexOf("/small-product-placeholder.png") < 0 && requestUrl.pathname.indexOf("/large-product-placeholder.png") < 0) return true;
  if (['/service-worker.js'].indexOf(requestUrl.pathname.pathname) > -1) return true;
  return false;
}

function isItServerRequest(path) {
  if (path == '' || path.split('.').length == 1) return true;else return false;
}
    ...
    if (checkResponse == true) {
      return fetch(request).then(function (networkResponse) {
        ...
        return cache.match(storageUrl).then(function (response) {
          if (response) return formFilter(response);else return fallBackResponse('live');
        });
      });
    } else {
      return cache.match(storageUrl).then(function (response) {
...
var firebaseConfig = {
  apiKey: "AIzaSyCkDVCb7WpJXcXFfyUS7pnd0Eg23NMzJHo",
  authDomain: "bagisto-pwa-6074e.firebaseapp.com",
  databaseURL: "https://bagisto-pwa-6074e-default-rtdb.firebaseio.com",
  projectId: "bagisto-pwa-6074e",
  storageBucket: "bagisto-pwa-6074e.appspot.com",
  messagingSenderId: "83254725065",
  appId: "1:83254725065:web:14c30b70dcbf98c8f34018"
